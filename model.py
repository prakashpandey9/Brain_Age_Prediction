...
def unet_model_3d(input_shape, downsize_filters_factor=1, pool_size=(2, 2, 2), n_labels=1,
    ...
    conv1 = Conv3D(int(8/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(inputs)
    conv1 = Conv3D(int(16/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(conv1)
    norm1 = BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001)(conv1)#??????????????????????
    pool1 = MaxPooling3D(pool_size=pool_size)(conv1)

    conv2 = Conv3D(int(16/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(pool1)
    conv2 = Conv3D(int(32/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(conv2)
    norm1 = BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001)(conv2)
    pool2 = MaxPooling3D(pool_size=pool_size)(conv2)

    conv3 = Conv3D(int(32/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(pool2)
    conv3 = Conv3D(int(64/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(conv3)
    norm1 = BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001)(conv3)
    pool3 = MaxPooling3D(pool_size=pool_size)(conv3)

    conv4 = Conv3D(int(64/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(pool3)
    conv4 = Conv3D(int(128/downsize_filters_factor), (3, 3, 3), activation='relu', strides=(1, 1, 1),
                   padding='same')(conv4)
